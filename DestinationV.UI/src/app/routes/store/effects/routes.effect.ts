...
export class RoutesEffects {
  ...
  deleteRoute$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(id => this.routesDataService.deleteRoute(id).pipe(
      map(x => {
        this.snackBarService.success('Route successfully deleted');
        return new DeleteRouteComplete(new ActionResult(null, true, false, id))
      }),
  ...
  updateRoute$: Observable<Action> = this.actions$.pipe(
    ...
    switchMap(routeItem => this.routesDataService.updateRoute(routeItem.id, routeItem).pipe(
      map(x => {
        this.snackBarService.success('Route successfully updated');
        return new UpdateRouteComplete(new ActionResult(null, true, false, routeItem))
      }),
