param(
    ...
    [string] $TargetDatabaseName = 'Destinationv',
...
if ($TargetServerName -eq '.\SQLEXPRESS') {
    ...
    $mdfPath = "$data_path\Destinationv.mdf"
    $ldfPath = "$data_path\Destinationv.ldf"

    # attach data files if they exist: 
    if ((Test-Path $mdfPath) -eq $true) {
        $sqlcmd = "IF DB_ID('DestinationV') IS NULL BEGIN CREATE DATABASE DestinationV ON (FILENAME = N'$mdfPath')"
...
& $SqlPackagePath  `
   /sf:DestinationV.Database.dacpac `
...
if ($TargetServerName -eq '.\SQLEXPRESS') {
    ...
    Write-Verbose "Deployed DestinationV database, data files at: $data_path"
